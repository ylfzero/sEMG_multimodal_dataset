...
_MODE_DIR = {0: "Static_gesture", 1: "Gesture_free"}


def _raw_path(mode: int, sub_id: int, day_id: int) -> str:
    return str(_ROOT / "data" / _MODE_DIR[mode] / f"Subject_{sub_id:02d}" / f"Day_{day_id}" / "raw.dat")
...
class Data_Manager:
    ...
    def read_data_file(self, path):
        """读取 raw.dat：帧布局 [ EMG | GLOVE | 2 尾列 ]，不含 IMU。"""
